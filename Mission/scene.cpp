...
bool FrameFunc()
{
	...
	{
		if (!path_requested && pathReadyCheck() && !lb_down)
		{
	...
	if (hge->Input_GetKeyState(HGEK_RBUTTON) && the_state == gsINPROGRESS)
	{
		...
		if (!path_requested && pathReadyCheck() && !rb_down)
		{
			...
			if (cellFlip(mx, my))
			{
				...
				if (the_way.path.length() > 0)
					wayNewRequest(the_way.target.x, the_way.target.y);
			}
			rb_down = true;
		}
	}
		...
		rb_down = false;
	if (path_requested && pathReadyCheck())
	{
...
////////////////////////////////////////////////////////////////////////////////
// Запрос обсчета пути
void wayNewRequest(int tx, int ty)
{
	int x, y;
...
{
	int x, y, dx, dy;
	the_way.path = pathRead();
	Field::pos_to_cell(&x, &y, the_character->position);
	if (the_way.path.length() > 0)
	{
		the_way.stage = 0;
		pathDirection(&dx, &dy, the_way.path.at(0));
		the_way.neighbour.x = x + dx;
		the_way.neighbour.y = y + dy;
